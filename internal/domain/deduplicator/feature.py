...
from internal.config.paths_config import ARTIFACTS_DIR, LEADS_SOURCED_PATH
from internal.domain.deduplicator.engine import DeduplicationEngine
...
def entrypoint() -> Dict:
    ...
    """
    logger.info("Loading raw prospects from %s", LEADS_SOURCED_PATH)
    
    # Load raw prospects
    try:
        with open(LEADS_SOURCED_PATH, 'r', encoding='utf-8') as f:
            raw_prospects: List[Dict[str, Any]] = json.load(f)
    except FileNotFoundError:
        logger.error("Raw prospects file not found: %s", LEADS_SOURCED_PATH)
        raise
